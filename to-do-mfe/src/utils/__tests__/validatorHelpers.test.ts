...
describe("validatorHelpers", () => {
  ...
  it("should throw error if not alphanumerics only", async () => {
    ...
    try {
      ...
      expect(error).toStrictEqual(new Error("Description allows only include alphanumeric characters"));
    }
  })
  ...
  it("should throw error if input has spaces only", async () => {
    ...
    try {
      ...
      expect(error).toStrictEqual(new Error("Description cannot be empty or without value."));
    }
  })

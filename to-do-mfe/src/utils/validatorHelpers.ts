...
export const checkInput = (_: any, value: string = ""): Promise<boolean | void> => {
  if (!value.replace(/\s/g, '')) return throwRejected("Description cannot be empty or without value.");
  if (isAlphanumeric(value)) 
    return throwRejected("Description allows only include alphanumeric characters");
